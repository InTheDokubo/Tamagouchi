...
const multiFound = await evaluate(`(()=>{Game.addCard('multi');let card=null;for(let i=0;i<16&&!card;i++){Game.spawnEnemy();card=Array.from(document.querySelectorAll('#hand-container>div')).find(el=>el.textContent.includes('連続拳'))}if(!card)return false;card.click();card.click();return true})()`);
if (!multiFound) throw new Error('Could not draw multi-hit test card');
await wait(1200);
const multiCombo = await evaluate(`Number(document.getElementById('combo-count').textContent)`);
if (multiCombo !== 2) throw new Error(`Multi-hit combo did not grow per hit: ${multiCombo}`);
...
await evaluate(`Game.addCard('quick_think'); Game.visitSecretMode(); true`);
await wait(760);
const secret = await evaluate(`({visible:!document.getElementById('scene-secret').classList.contains('hidden'),cards:document.getElementById('secret-card-list').children.length,quick:Array.from(document.getElementById('secret-card-list').children).find(el=>el.textContent.includes('思考加速'))?.textContent||''})`);
if (!secret.visible || secret.cards < 1 || !secret.quick.includes('静心の守り') || secret.quick.includes('輪廻刻印')) throw new Error(`Secret mode failed: ${JSON.stringify(secret)}`);
...
await evaluate(`Game.selectType('hp'); document.getElementById('input-name').value='体力'; Game.start(); true`);
await wait(700);
await evaluate(`Game.initBattle(); true`);
await waitFor(`!document.getElementById('scene-battle').classList.contains('hidden')`);
await waitFor(`!document.getElementById('battle-block').classList.contains('hidden')`);
const hpTrait = await evaluate(`({block:Number(document.getElementById('battle-block-val').textContent),visible:!document.getElementById('battle-block').classList.contains('hidden')})`);
if (!hpTrait.visible || hpTrait.block < 6) throw new Error(`HP trait did not provide its opening shell: ${JSON.stringify(hpTrait)}`);
await evaluate(`Game.returnToStart(true); true`);
await wait(700);
await evaluate(`Game.selectType('int'); document.getElementById('input-name').value='魔力'; Game.start(); true`);
await wait(700);
await evaluate(`Game.initBattle(); true`);
await waitFor(`!document.getElementById('scene-battle').classList.contains('hidden')`);
await wait(300);
const intPreview = await evaluate(`(()=>{let card=null;for(let i=0;i<12&&!card;i++){card=Array.from(document.querySelectorAll('#hand-container>div')).find(el=>el.textContent.includes('プチ魔法'));if(!card)Game.spawnEnemy()}if(!card)return '';card.click();return card.querySelector('.card-preview')?.textContent||''})()`);
const intDamage = Number(intPreview.match(/予測\s+(\d+)/)?.[1] || 0);
if (intDamage < 10) throw new Error(`INT spell charge bonus missing: ${intPreview}`);
